			...
			for (int y = 0; y < 16; y++) {
				...
				if (mod(inPos.getX(), thickness) == 0 && mod(inPos.getZ(), thickness) == 0) {
					...
					if (this.mazes.containsKey(mazePos)) {
						...
						maze = mazeCreator.newMaze(mazePos, redundancy ? width + 4 : width, redundancy ? height + 4 : height, RandomGenerator.createLegacy(blockSeed(mazePos.getX(), mazePos.getZ(), seed + seedModifier)));
